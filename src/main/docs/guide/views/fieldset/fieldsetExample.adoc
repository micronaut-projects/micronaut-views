...
[.lang-python]
--
NOTE: The Python controller is annotated with `@Validated` so that the `@Valid` body is validated: the Python compiler only applies around advice to a class that carries an `@Around` stereotype explicitly.
--
