...
After configuring Micronaut and wiring up your `SoyFileSetProvider`:
...
public class CustomSoyFileSetProvider implements SoyFileSetProvider {
  ...
  public SoyFileSet provideSoyFileSet() {
    ...
    final ClassLoader classLoader = CustomSoyFileSetProvider.class.getClassLoader();
    for (final String template : VIEWS) {
      final URL resource = classLoader.getResource(template);
      if (resource != null) {
...
you can render templates like so:
