...
The last step is to provide the renaming map to Micronaut Views Soy with a bean of type api:views.soy.SoyNamingMapProvider[]:

snippet::io.micronaut.views.docs.soy.RewriteMapProvider[tags="clazz"]

[.lang-python]
--
NOTE: `cssRenamingMap()` overrides a default method of the `SoyNamingMapProvider` interface; the Python compiler only bridges abstract interface methods to Java automatically, so the Python method is annotated with `@Executable`.
--
