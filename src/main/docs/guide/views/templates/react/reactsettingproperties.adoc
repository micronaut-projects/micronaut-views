...
This makes the server pick up a rebuilt bundle without a restart. It does not tell the browser, so the
refresh is still manual — see <<reactdevreload,Refreshing the browser on rebuild>> for the module that
closes that gap.
