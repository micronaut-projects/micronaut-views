...
Micronaut Views exposes a Micronaut-native Thymeleaf `#request` expression object for request metadata such as
`requestURI`, `requestURL`, `contextPath`, `method`, `path`, and `queryString`.
This object is backed by Micronaut's api:http.HttpRequest[] and is not a servlet request.
Servlet-specific expression objects such as `#response` and `#servletContext` are not provided.
When Micronaut's server request contains only a relative URI, `requestURL` also renders that relative URI path.
