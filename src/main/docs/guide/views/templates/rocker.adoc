...
NOTE: Rocker compiles each template to a Java class (`views.home` for `views/home.rocker.html`), so this example is Java only.

snippet::io.micronaut.views.rocker.docs.HomeController[tags="clazz",project="views-rocker",language="java"]
