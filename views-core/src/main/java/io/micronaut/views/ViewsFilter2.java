 ...
 * @since 5.9.0
 */
@Requires(beans = ViewsResolver.class, classes = MutableHttpResponse.class)
@ServerFilter(ServerFilter.MATCH_ALL_PATTERN)
public class ViewsFilter2 implements Ordered {
    private static final Logger LOG = LoggerFactory.getLogger(ViewsFilter2.class);
     ...
     */
    @Inject
    public ViewsFilter2(ViewsResolver viewsResolver,
    ...
    @ResponseFilter
    final MutableHttpResponse<?> filterResponse(HttpRequest<?> request, MutableHttpResponse<?> response) {
        ...
        if (writableOptional.isPresent()) {
            ...
            response.contentType(MediaType.TEXT_HTML_TYPE);
            return response;
        }
    ...
    protected MediaType resolveMediaType(@Nullable HttpRequest<?> request, @NonNull HttpResponse<?> response, @Nullable Object responseBody) {
        Optional<RouteMatch<?>> routeMatch = RouteAttributes.getRouteMatch(response);
        if (routeMatch.isEmpty()) {
            return MediaType.APPLICATION_JSON_TYPE;
        }
        AnnotationMetadata route = (AnnotationMetadata) routeMatch.get();
