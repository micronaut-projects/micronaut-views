...
public class FreemarkerTest {
    ...
    @Test
    void testFreemarker() {
        HttpRequest<?> request = HttpRequest.GET("/views/freemarker");
        HttpResponse<String> response = client.toBlocking().exchange(request, String.class);
        assertNotNull(response);
        Optional<String> body = response.getBody();
        assertTrue(body.isPresent());
        assertTrue(body.get().contains("Title - Freemarker"));
        assertTrue(body.get().contains("username: <span>test-user</span>"));
    }

    @Test
    void testFreemarkerPojoView() {
        HttpRequest<?> request = HttpRequest.GET("/views/freemarker-view");
        HttpResponse<String> response = client.toBlocking().exchange(request, String.class);
        assertNotNull(response);
        Optional<String> body = response.getBody();
        assertTrue(body.isPresent());
