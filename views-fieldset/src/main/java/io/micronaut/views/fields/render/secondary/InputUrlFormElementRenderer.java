...
@Secondary
@Singleton
public class InputUrlFormElementRenderer implements FormElementRenderer<InputUrlFormElement> {
    ...
    @Override
    @NonNull
    public String render(@NonNull InputUrlFormElement el,
                         @NonNull Locale locale) {
        StringBuilder sb = new StringBuilder();
        if (el.label() != null) {
            sb.append(renderLabel(el.id(), el.label(), messageSource, locale));
        }
        sb.append(render(HtmlTag.INPUT, attributes(el, InputType.URL)));
        return sb.toString();
    }
}
